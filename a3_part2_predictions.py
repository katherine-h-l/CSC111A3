...
class SimilarUserPredictor(ReviewScorePredictor):
    ...
    def predict_review_score(self, user: str, book: str) -> int:
        ...
        Preconditions:
            ...
            - book in self.graph._vertices
        """
        if self.graph.adjacent(book, user):
            return self.graph.get_weight(book, user)
        else:
            readers = self.graph.get_neighbours(book)

            total_weights = 0
            total_score = 0
            for reader in readers:
                weight = self.graph.get_similarity_score(reader, user, self._score_type)

                total_weights += weight
                total_score += weight * self.graph.get_weight(book, reader)

            return round(total_score / total_weights)
...
def evaluate_predictor(predictor: ReviewScorePredictor,
    ...
    with open(test_file) as test_file_csv:
        reviews = csv.reader(test_file_csv)
        ...
        for review in reviews:
            num_reviews += 1
            prediction = predictor.predict_review_score(review[0], book_names[review[1]])
            if prediction == int(review[2]):
                num_correct += 1
            else:
                total_error += abs(prediction - int(review[2]))
