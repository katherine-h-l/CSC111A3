...
class _Vertex:
    ...
    def similarity_score(self, other: _Vertex) -> float:
        ...
        if self.degree() == 0 or other.degree() == 0:
            ...
            # set.union doesn't mutate
            adjacent_either = len(self.neighbours.union(other.neighbours))
...
class Graph:
    ...
    def recommend_books(self, book: str, limit: int) -> list[str]:
        ...
        *descending order* of similarity score. Ties are broken in descending order
        ...
        """
        return sorted(
            (item for item in self.get_all_vertices('book') if item != book
             and self.get_similarity_score(book, item) != 0),
            key=lambda a: self.get_similarity_score(book, a),
            reverse=True
        )[:limit]
...
def load_review_graph(reviews_file: str, book_names_file: str) -> Graph:
    ...
    with open(reviews_file) as reviews_file_csv:
        ...
        for review in reviews:
            ...
            graph.add_edge(review[0], book_names[review[1]])
